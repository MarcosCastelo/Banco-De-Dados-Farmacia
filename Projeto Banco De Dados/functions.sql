...
CREATE OR REPLACE FUNCTION realiza_venda (id_venda INTEGER cpf VARCHAR(11), id_prod INT, quant INT, local_nome VARCHAR(50)) RETURNS VOID AS $$
	DECLARE
		id_loja INT;
	...
	BEGIN
		...
		IF EXISTS(SELECT VENDA_ID FROM VENDA WHERE VENDA_ID = ID_VENDA) THEN
			UPDATE VENDA SET HORA_DATE = NOW() WHERE VENDA_ID = ID_VENDA;
		ELSE
			...
			INSERT INTO VENDA VALUES(ID_VENDA, CPF, ID_LOCAL, NOW());

			SELECT MAX(*) + 1 INTO ID_ITEM_VENDA FROM ITEM_VENDA;
			IF ID_ITEM_VENDA IS NULL THEN
				ID_ITEM_VENDA := 1;
			END IF;
			
			INSERT INTO ITEM_VENDA(ID_ITEM_VENDA, ID_VENDA);
		END IF;
	END
	$$ LANGUAGE 'plpgsql';
