...
select realiza_venda(1, '07069895380', 'RIVOTRIL', 2, 'images/receita01', 'PORTO ALEGRE')
SELECT * FROM VENDA

CREATE OR REPLACE FUNCTION realiza_venda (id_venda INTEGER, CLI_cpf VARCHAR(11), nome_prod VARCHAR(50), quant INT, NOME_receita VARCHAR(50), local_nome VARCHAR(50)) RETURNS VOID AS $$
	DECLARE
		id_loja INT;
	...
	BEGIN
		...
		IF CAT_PROD = 'PRETA' AND NOME_RECEITA IS NULL OR CAT_PROD = 'VERMELHA' AND NOME_RECEITA IS NULL THEN
		...
		SELECT PRECO INTO VALOR_PRODUTO FROM ESTOQUE WHERE ESTOQUE_ID = ID_ESTOQUE;
		SELECT CLIENTE_ID INTO ID_CLIENTE FROM CLIENTE WHERE CLIENTE.CPF LIKE CLI_CPF;
		
		IF EXISTS(SELECT VENDA_ID FROM VENDA WHERE VENDA_ID = ID_VENDA) THEN
			UPDATE VENDA SET HORA_DATE = NOW(), VALOR_TOTAL = VALOR_PRODUTO * QUANT + VALOR_ENTREGA + VALOR_TOTAL WHERE VENDA_ID = ID_VENDA;
		...
		ELSE
			SELECT MAX(VENDA_ID) + 1 INTO ID_V FROM VENDA;
			IF ID_V IS NULL THEN
				ID_V := 1;
			END IF;
			
			INSERT INTO VENDA VALUES(ID_VENDA, ID_CLIENTE, ID_LOCAL, NOW(), VALOR_PRODUTO * QUANT + VALOR_ENTREGA);
			INSERT INTO ITEM_VENDA VALUES (DEFAULT, ID_VENDA, ID_ESTOQUE, QUANT, VALOR_PRODUTO);
			UPDATE ESTOQUE SET QUANTIDADE = QUANTIDADE - QUANT WHERE ESTOQUE_ID = ID_ESTOQUE;
		END IF;
	END
